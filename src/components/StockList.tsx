...
export default function StockList({ items, onViewDetails, searchTerm = "" }: StockListProps) {
  ...
  const [expandedLocations, setExpandedLocations] = useState<Record<string, boolean>>({}); // NEW
  ...
  const toggleLocation = (cat: string, type: string, loc: string) => setExpandedLocations(p => ({ ...p, [`${cat}-${type}-${loc}`]: !p[`${cat}-${type}-${loc}`] }));
  ...
  return (
    <div className="space-y-4 pb-20">
      {Object.entries(groupedItems).map(([categoryName, types]) => {
        ...
        // Count total items deep inside
        const totalItemsInCategory = Object.values(types).reduce((sum, locs) => 
          sum + Object.values(locs).flat().length, 0
        );
          ...
          <div key={categoryName} className="animate-fade-in-up overflow-hidden rounded-3xl bg-white shadow-sm ring-1 ring-stone-100">
            ...
            {isCategoryOpen && (
              <div className="border-t border-stone-100 px-2 pb-2 pt-2 sm:px-4">
                {Object.entries(types).map(([typeName, locations]) => {
                  const typeKey = `${categoryName}-${typeName}`;
                  const isTypeOpen = isSearching || expandedTypes[typeKey] || false;
                  const totalItemsInType = Object.values(locations).flat().length;
                  ...
                  return (
                    <div key={typeName} className="mb-2 overflow-hidden rounded-2xl border border-stone-100 bg-white last:mb-0">
                      ...
                      <button onClick={() => toggleType(categoryName, typeName)} className="flex w-full items-center justify-between bg-stone-50/30 px-5 py-3 transition-colors hover:bg-stone-50">
                        <h3 className="text-xs font-bold uppercase tracking-widest text-stone-500">{typeName} ({totalItemsInType})</h3>
                      ...
                      {isTypeOpen && (
                        <div className="border-t border-stone-100">
                          {Object.entries(locations).map(([locName, stockItems]) => {
                            const locKey = `${categoryName}-${typeName}-${locName}`;
                            const isLocOpen = isSearching || expandedLocations[locKey] || false;

                            return (
                              <div key={locName} className="border-b border-stone-50 last:border-0">
                                {/* LEVEL 3: LOCATION */}
                                <button onClick={() => toggleLocation(categoryName, typeName, locName)} className="flex w-full items-center justify-between bg-stone-50/10 px-6 py-2 transition-colors hover:bg-stone-50">
                                  <div className="flex items-center gap-2">
                                    <span className="h-1.5 w-1.5 rounded-full bg-orange-400"></span>
                                    <span className="text-[10px] font-bold uppercase tracking-wider text-stone-400">{locName}</span>
                                  </div>
                                  <svg className={`h-3 w-3 text-stone-300 transition-transform duration-200 ${isLocOpen ? 'rotate-180' : ''}`} fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M19 9l-7 7-7-7" /></svg>
                                </button>
