...
export default function StockList({ items, onViewDetails }: StockListProps) {
  ...
  const toggleType = (categoryName: string, typeName: string) => {
  ...
  return (
    <div className="space-y-4 pb-20">
      {Object.entries(groupedItems).map(([categoryName, types]) => {
        const isCategoryOpen = expandedCategories[categoryName] || false;
        const totalItemsInCategory = Object.values(types).flat().length;
        ...
        return (
          <div key={categoryName} className="animate-fade-in-up overflow-hidden rounded-3xl bg-white shadow-sm ring-1 ring-stone-100">
            ...
            <button 
              ...
              className="flex w-full items-center justify-between bg-stone-50/50 p-6 transition-colors hover:bg-stone-50"
            >
              ...
              <svg 
                className={`h-5 w-5 text-stone-400 transition-transform duration-200 ${isCategoryOpen ? 'rotate-180' : ''}`} 
                ...
                {Object.entries(types).map(([typeName, stockItems]) => {
                  ...
                  return (
                    <div key={typeName} className="mb-2 overflow-hidden rounded-2xl border border-stone-100 bg-white last:mb-0">
                      ...
                      <button 
                        onClick={() => toggleType(categoryName, typeName)}
                        className="flex w-full items-center justify-between bg-stone-50/30 px-5 py-3 transition-colors hover:bg-stone-50"
                      >
                        <h3 className="text-xs font-bold uppercase tracking-widest text-stone-500">
                          {typeName} ({stockItems.length})
                        </h3>
                        <svg 
                          className={`h-4 w-4 text-stone-400 transition-transform duration-200 ${isTypeOpen ? 'rotate-180' : ''}`} 
                          fill="none" viewBox="0 0 24 24" stroke="currentColor"
                        >
                          <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M19 9l-7 7-7-7" />
                        </svg>
