...
export default function Dashboard() {
  ...
  const validInventory = inventory.filter(item => item.category && item.category.toLowerCase() !== "uncategorized");

  const lowStockItems = validInventory.filter(item => item.quantity <= (item.minStock ?? 5));
  
  const filteredItems = validInventory.filter(item => {
  ...
  return (
    <Layout>
      <div className="mb-8 mt-2">
        ...
        <p className="mt-2 text-stone-500 font-medium">{validInventory.length} items across all categories.</p>
      </div>
